...
const SelectLayout = ({ darkMode }) => {
    ...
    <div className="px-6 py-4 relative" style={{ background: 'linear-gradient(135deg, #1a2e1a, #0f1f0f)' }}>
      <div className="w-20 h-20 rounded-2xl overflow-hidden absolute -top-10 left-6 border-4" style={{ borderColor: '#0f1f0f', background: '#2a3f2a' }}>
        {userData?.photoURL ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-white text-xl font-bold" style={{ background: '#3a5a3a' }}>
      ...
      <div className="pt-12">
        ...
        {userData?.bio && (
          <p className="text-sm leading-relaxed mb-4 pb-4" style={{ color: 'rgba(255,255,255,0.7)', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
            {userData.bio}
          </p>
        )}
        {userData?.skills && (
          <div className="mb-4 pb-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
            <h3 className="text-xs font-bold mb-2 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.5)' }}>
              Expertise
            </h3>
            <div className="flex flex-wrap gap-2">
              {userData.skills.split(',').slice(0, 6).map((s, i) => (
                <span key={i} className="px-2 py-1 rounded-lg text-xs" style={{ background: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.85)' }}>
                  {s.trim()}
                </span>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(userData.socialLinks).filter(([, v]) => v).map(([p, url]) => (
                <a key={p} href={url} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.7)' }}>
                  {getSocialIcon(p)}
                </a>
              ))}
            </div>
    ...
    <div className="px-6 py-4 relative" style={{ background: '#0d1b2e' }}>
      <div className="w-20 h-20 rounded-2xl overflow-hidden absolute -top-10 left-6" style={{ border: '4px solid #0d1b2e', background: 'linear-gradient(135deg, #1a3a5c, #0d1b2e)' }}>
        {userData?.photoURL ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-white text-xl font-bold" style={{ background: '#1a3a5c' }}>
            {getInitials(userData?.displayName)}
          </div>
        )}
      </div>
      <div className="pt-12">
        ...
        {userData?.company && (
          <div className="flex items-center gap-2 text-xs mb-4" style={{ color: '#5a8ab0' }}>
        ...
        {userData?.skills && (
          <div className="mb-4 pb-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
            <h3 className="text-xs font-bold mb-2 uppercase tracking-wider" style={{ color: '#5a8ab0' }}>
              Expertise
            </h3>
            <div className="flex flex-wrap gap-2">
              {userData.skills.split(',').slice(0, 6).map((s, i) => (
                <span key={i} className="px-2 py-1 rounded-lg text-xs" style={{ background: 'rgba(255,255,255,0.06)', color: '#93b4d4' }}>
                  {s.trim()}
                </span>
              ))}
            </div>
        ...
        {Object.entries(userData?.socialLinks || {}).some(([, v]) => v) && (
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(userData.socialLinks).filter(([, v]) => v).map(([p, url]) => (
                <a key={p} href={url} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.06)', color: '#93b4d4' }}>
                  {getSocialIcon(p)}
                </a>
              ))}
            </div>
  ...
  <div className="w-full font-['Inter'] text-white" style={{ background: 'linear-gradient(160deg, #2a3a2a, #1a2a1e)' }}>
    <div className="flex flex-col items-center pt-8 pb-4 px-4">
      ...
      <p className="text-xs mb-2" style={{ color: 'rgba(255,255,255,0.7)' }}>{userData?.occupation}{userData?.occupation && userData?.company ? ' | ' : ''}{userData?.company}</p>
      
      {/* Bio */}
      {userData?.bio && (
        <p className="text-xs text-center mb-4" style={{ color: 'rgba(255,255,255,0.6)' }}>{userData.bio}</p>
      )}
      ...
      {userData?.skills && (
        <div className="w-full mb-4">
          <h3 className="text-xs font-bold mb-2 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.5)' }}>Expertise</h3>
          <div className="flex flex-wrap gap-2">
            {userData.skills.split(',').slice(0, 6).map((s, i) => (
              <span key={i} className="px-2 py-1 rounded-lg text-xs" style={{ background: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.8)' }}>{s.trim()}</span>
            ))}
          </div>
        </div>
      )}
      ...
      <div className="w-full space-y-2">
        {/* Phone */}
        {userData?.phoneNumber && (
          <a href={`tel:${userData.phoneNumber}`} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-white no-underline" style={{ background: 'rgba(255,255,255,0.08)' }}>
            <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.12)' }}><FaPhone /></div>
            <div className="flex-1"><div className="text-xs font-medium">Call me</div><div className="text-[11px] opacity-60">{userData.phoneNumber}</div></div>
        ...
        {/* Email */}
        {userData?.email && (
          <a href={`mailto:${userData.email}`} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-white no-underline" style={{ background: 'rgba(255,255,255,0.08)' }}>
            <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.12)' }}><FaEnvelope /></div>
            <div className="flex-1"><div className="text-xs font-medium">Email me</div><div className="text-[11px] opacity-60">{userData.email}</div></div>
            <span className="text-xs opacity-40">↗</span>
          </a>
        )}
        ...
        {Object.entries(userData?.socialLinks || {}).filter(([, v]) => v).map(([p, url]) => (
          <a key={p} href={url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-white no-underline" style={{ background: 'rgba(255,255,255,0.08)' }}>
            <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.12)' }}>{getSocialIcon(p)}</div>
            <div className="flex-1"><div className="text-xs font-medium capitalize">Follow me</div><div className="text-[11px] opacity-60">{url.replace(/https?:\/\/(www\.)?/, '').split('/')[0]}</div></div>
            <span className="text-xs opacity-40">↗</span>
          </a>
        ))}
      </div>
  ...
  <div className="w-full font-['Inter'] text-white" style={{ background: '#0f1623' }}>
    <div className="flex flex-col items-center pt-8 pb-4 px-4">
      ...
      <h1 className="text-xl font-medium text-white mb-1">{userData?.displayName}</h1>
      <p className="text-xs mb-2" style={{ color: 'rgba(255,255,255,0.6)' }}>{userData?.occupation}{userData?.occupation && userData?.company ? ' | ' : ''}{userData?.company}</p>
      
      {/* Bio */}
      {userData?.bio && (
        <p className="text-xs text-center mb-4" style={{ color: 'rgba(255,255,255,0.5)' }}>{userData.bio}</p>
      )}
      ...
      {userData?.skills && (
        <div className="w-full mb-4">
          <h3 className="text-xs font-bold mb-2 uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.4)' }}>Expertise</h3>
          <div className="flex flex-wrap gap-2">
            {userData.skills.split(',').slice(0, 6).map((s, i) => (
              <span key={i} className="px-2 py-1 rounded-lg text-xs" style={{ background: 'rgba(255,255,255,0.08)', border: '0.5px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.7)' }}>{s.trim()}</span>
            ))}
          </div>
        </div>
      )}
      ...
      <div className="w-full space-y-2">
        {/* Phone */}
        {userData?.phoneNumber && (
          <a href={`tel:${userData.phoneNumber}`} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-white no-underline" style={{ background: 'rgba(255,255,255,0.06)', border: '0.5px solid rgba(255,255,255,0.08)' }}>
            <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.1)' }}><FaPhone /></div>
            <div className="flex-1"><div className="text-xs font-medium">Call me</div><div className="text-[11px] opacity-55">{userData.phoneNumber}</div></div>
        ...
        {/* Email */}
        {userData?.email && (
          <a href={`mailto:${userData.email}`} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-white no-underline" style={{ background: 'rgba(255,255,255,0.06)', border: '0.5px solid rgba(255,255,255,0.08)' }}>
            <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.1)' }}><FaEnvelope /></div>
            <div className="flex-1"><div className="text-xs font-medium">Email me</div><div className="text-[11px] opacity-55">{userData.email}</div></div>
            <span className="text-xs opacity-35">↗</span>
          </a>
        )}
  ...
  <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.08)' }}>
    ...
    <div className="flex-1"><div className="text-xs font-medium">Location</div></div>
        ...
        {/* Social Links */}
        {Object.entries(userData?.socialLinks || {}).filter(([, v]) => v).map(([p, url]) => (
          <a key={p} href={url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-white no-underline" style={{ background: 'rgba(255,255,255,0.06)', border: '0.5px solid rgba(255,255,255,0.08)' }}>
            <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.1)' }}>{getSocialIcon(p)}</div>
            <div className="flex-1"><div className="text-xs font-medium capitalize">Follow me</div><div className="text-[11px] opacity-55">{url.replace(/https?:\/\/(www\.)?/, '').split('/')[0]}</div></div>
            <span className="text-xs opacity-35">↗</span>
          </a>
        ))}
      </div>
  ...
  <div className="w-full bg-white font-['Inter']">
    <div className="flex flex-col items-center pt-8 pb-4 px-4">
      ...
      <p className="text-xs text-gray-500 mb-2">{userData?.occupation}{userData?.occupation && userData?.company ? ' | ' : ''}<span className="underline">{userData?.company}</span></p>
      ...
      {userData?.skills && (
        <div className="w-full mb-4">
          <h3 className="text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">Expertise</h3>
          <div className="flex flex-wrap gap-2">
            {userData.skills.split(',').slice(0, 6).map((s, i) => (
              <span key={i} className="px-2 py-1 bg-gray-100 text-gray-700 rounded-lg text-xs border border-gray-200">{s.trim()}</span>
            ))}
          </div>
        </div>
      )}
      ...
      <div className="w-full space-y-2">
        {/* Phone */}
        {userData?.phoneNumber && (
          <a href={`tel:${userData.phoneNumber}`} className="flex items-center gap-3 px-3 py-2.5 rounded-xl no-underline" style={{ background: '#f9fafb', border: '0.5px solid #e5e7eb' }}>
            <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-green-500"><FaPhone className="text-white text-xs" /></div>
            <div className="flex-1"><div className="text-xs font-medium text-gray-900">Call me</div><div className="text-[11px] text-gray-500">{userData.phoneNumber}</div></div>
        ...
        {/* Email */}
        {userData?.email && (
          <a href={`mailto:${userData.email}`} className="flex items-center gap-3 px-3 py-2.5 rounded-xl no-underline" style={{ background: '#f9fafb', border: '0.5px solid #e5e7eb' }}>
            <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-blue-500"><FaEnvelope className="text-white text-xs" /></div>
            <div className="flex-1"><div className="text-xs font-medium text-gray-900">Email me</div><div className="text-[11px] text-gray-500">{userData.email}</div></div>
            <span className="text-xs text-gray-400">↗</span>
          </a>
        )}
    ...
    <div className="flex-1"><div className="text-xs font-medium">Location</div></div>
        ...
        {Object.entries(userData?.socialLinks || {}).filter(([, v]) => v).map(([p, url]) => (
          <a key={p} href={url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-3 py-2.5 rounded-xl no-underline" style={{ background: '#f9fafb', border: '0.5px solid #e5e7eb' }}>
            <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-blue-600"><span className="text-white text-xs">{getSocialIcon(p)}</span></div>
            <div className="flex-1"><div className="text-xs font-medium text-gray-900 capitalize">Follow me</div><div className="text-[11px] text-gray-500">{url.replace(/https?:\/\/(www\.)?/, '').split('/')[0]}</div></div>
            <span className="text-xs text-gray-400">↗</span>
          </a>
        ))}
      </div>
